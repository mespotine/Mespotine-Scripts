...
-- Import render-settings from a project-file into a render-preset
...
if RenderTable==nil then 
  ...
  retval, presetname = reaper.GetUserInputs("Enter Render Preset Name", 1, "Presetname", "")
  if retval==false then return end
  --retval, dirty = ultraschall.ApplyRenderTable_Project(RenderTable)
  RenderTable2 = ultraschall.GetRenderPreset_RenderTable(presetname, presetname)
  if RenderTable2~=nil then 
    retval=reaper.MB("Preset already exists.\n\nShall I replace it ?", "Preset already exists", 4)
    if retval~=6 then return end
     retval = ultraschall.SetRenderPreset(presetname, presetname, RenderTable)
  else
    retval = ultraschall.AddRenderPreset(presetname, presetname, RenderTable)
  end
end
