...
-- snap RazorEdit-start to item-start at mouse position
...
if item~=nil then
  ...
  if Envelope==nil then
    ...
    retval = ultraschall.RazorEdit_Set_Track(MediaTrack, razor_edit_index, start, end_position)
  else
    --altered_razor_edit_string = ultraschall.RazorEdit_Add_Envelope(Envelope, start, stop)
    retval = ultraschall.RazorEdit_Set_Envelope(Envelope, razor_edit_index, start, end_position)
  end
end

reaper.Undo_EndBlock("Snap RazorArea To Item Start", -1)
