...
-- snap a RazorEdit to the item at mouse position
...
if item~=nil then
  ...
  if Envelope==nil then
    ...
    retval = ultraschall.RazorEdit_Set_Track(MediaTrack, razor_edit_index, start, stop)
  else
    --altered_razor_edit_string = ultraschall.RazorEdit_Add_Envelope(Envelope, start, stop)
    retval = ultraschall.RazorEdit_Set_Envelope(Envelope, razor_edit_index, start, stop)
  end
end

reaper.Undo_EndBlock("Snap RazorEdit to Item", -1)
